$(document).ready(function(){
	...
	$("#goNext").click(function(){
		...
		//결제금액에 형식 지정
		totalAmt = fn_thousandSeparatorCommas(totalAmt);
		...
		//예약번호 발급 및 step3 결제정보에 예약번호 표현
		fn_selectBookNum(cinemaNum);
		...
		success : function(data){
			...
			for(var i=0; i<subLocation.length; i++){
				$("#ul_subLocation").append(
						'<li>'+
							'<a id="subLocation'+subLocation[i].cinemaNum+'" href="#">'+subLocation[i].cinemaName+'</a>'+
			...
			$("a[id^='subLocation']").click(function(){
	 			...
	 			var hidden_cinemaName = $("input[id^='"+substringId+"hidden_cinemaName']").val();
	 			$("#cinemaName").val(hidden_cinemaName);
	 			
	 			//지점을 선택 시 영화 상영 정보를 조회
	 			fn_setMovieShowInfo();
			})
		}
	})
	...
	$.ajax({
        ...
        success : function(findBookNum){
        	...
        	if(jsonValue.substring(13,17) == "null"){
            	...
            	$("#txt_bookNum").text("예매번호 : "+jsonResultValue);
        	}
        }
    })
}